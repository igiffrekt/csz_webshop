...
depends_on: [08-01]
files_modified:
  - apps/cms/src/api/quote-request/content-types/quote-request/lifecycles.ts
autonomous: true

must_haves:
  truths:
    - "User receives email when quote request submitted"
    - "Lifecycle hooks log quote status changes"
  artifacts:
    - path: "apps/cms/src/api/quote-request/content-types/quote-request/lifecycles.ts"
      provides: "Quote request lifecycle hooks"
      contains: "afterCreate"
---

<objective>
Add email notifications and lifecycle hooks for quote requests.

Purpose: QUOT-03 (user receives confirmation when submitted). Enhance admin workflow.

Output: Lifecycle hooks that send confirmation email and log status changes.
</objective>

<tasks>

<task type="auto">
  <name>Task 1: Create quote request lifecycle hooks</name>
  <files>
    apps/cms/src/api/quote-request/content-types/quote-request/lifecycles.ts
  </files>
  <action>
Create lifecycle hooks at `apps/cms/src/api/quote-request/content-types/quote-request/lifecycles.ts`:
  ...
  async afterCreate(event) {
    ...
    try {
      ...
      const itemsList = items
        .map((item: { productName: string; quantity: number; variantName?: string }) => {
...
```
  ...
  <verify>
    - afterCreate sends confirmation email
    - afterUpdate logs status changes
    - Email sent when status becomes 'quoted'
  </verify>
  <done>Lifecycle hooks created</done>
</task>
...
<task type="auto">
  ...
  <files>none</files>
  <action>
Build CMS to verify lifecycle hooks compile:

```bash
pnpm --filter @csz/cms build
```

Verify no TypeScript errors in lifecycle file.
  </action>
  <verify>
...
1. Lifecycle hooks file exists
2. afterCreate sends confirmation email
3. afterUpdate sends notification when status is 'quoted'
4. CMS builds successfully
...
<success_criteria>
- Quote request lifecycle hooks in place
- Confirmation email sent on create
- Status change notification when quoted
- CMS builds without errors
</success_criteria>
