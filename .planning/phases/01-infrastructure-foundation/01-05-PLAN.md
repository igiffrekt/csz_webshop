...
must_haves:
  ...
  artifacts:
    - type: "database-state"
      description: "Custom admin roles stored in Strapi database (admin_roles table)"
      verification: "Navigate to Settings > Administration Panel > Roles in Strapi admin"
...
Document the credentials in a secure location (not in git).
  ...
  <resume-signal>Type "done" after creating Super Admin account and logging in</resume-signal>
</task>
...
Note: Some collection types (Product, Category, etc.) don't exist yet - they'll be created in Phase 2. The role will be updated then to grant specific permissions.
  ...
  <resume-signal>Type "done" after creating Store Manager role</resume-signal>
</task>
