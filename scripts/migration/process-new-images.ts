/**
 * Process ONLY NEW (unprocessed) Product Images with Cloudinary
 *
 * This script:
 * 1. Fetches product images from Strapi that haven't been processed yet
 ...
 *
 * Usage:
 *   npx tsx scripts/migration/process-new-images.ts
 */
...
async function getUnprocessedProducts(): Promise<StrapiProduct[]> {
  console.log('Fetching products with unprocessed images...');
  ...
  while (true) {
    ...
    for (const product of data.data) {
      if (product.images && product.images.length > 0) {
        ...
        // Skip if already processed (has _transparent in name)
        if (image.name && image.name.includes('_transparent')) {
          continue;
        }
        products.push({
          id: product.id,
  ...
  console.log(`  Found ${products.length} products with unprocessed images\n`);
  return products;
}
...
async function main(): Promise<void> {
  console.log('╔════════════════════════════════════════════╗');
  console.log('║   Process NEW Images (Background + Shadow) ║');
  console.log('╚════════════════════════════════════════════╝\n');
  ...
  const products = await getUnprocessedProducts();

  if (products.length === 0) {
    console.log('✅ All products already have processed images!');
    return;
  }

  console.log(`Processing ${products.length} new images...\n`);
  ...
  for (const product of products) {
    try {
      ...
      console.log(`\n[${processed + failed + 1}/${products.length}] ${product.name}`);
      ...
      try {
        ...
        console.log(`  ✗ Shadow error: ${shadowErr}`);
        failed++;
        continue;
      }
